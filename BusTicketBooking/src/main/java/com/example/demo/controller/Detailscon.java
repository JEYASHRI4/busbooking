...
@GetMapping("showquerydetails/{email}")
public List<Details> getinfobyemail(@PathVariable String email){
	return se.getperdetails(email);
}
@DeleteMapping("deletequeryid/{id}")
public String delinfo(@PathVariable String id) {
	se.delperinfo(id);
	return id;
}
@PutMapping("updatequery/{newname}/{oldid}")
public String updateinfo(@PathVariable String newname,@PathVariable String oldid) {
